...
<div id="whitebox">
	...
	<?php foreach ($roles as $r) {
		if(in_array($r->role_id, $db_roles)){
			if($r->role_id==0)
				echo '<input type="checkbox" class="checkbox3" value='.$r->role_id.' name="role[]" checked />'.$r->description;
			else
				echo '<input type="checkbox" class="checkbox2" value='.$r->role_id.' name="role[]" checked />'.$r->description;
		}
		else{
			if($r->role_id==0)
				echo '<input type="checkbox" class="checkbox3" value='.$r->role_id.' name="role[]" />'.$r->description;
			else
